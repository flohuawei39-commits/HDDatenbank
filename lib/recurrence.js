...
const vorkommen = (entry, von, bis) => {
  ...
  // Ausnahmen: einzelne Vorkommen, die diesmal ausfallen. Die Serie bleibt.
  const ausnahmen = new Set(Array.isArray(wdh.ausnahmen) ? wdh.ausnahmen : []);
  const treffer = [];
  ...
  for (let i = 0; i < 800; i += 1) {
    ...
    if (stop >= von && !ausnahmen.has(start)) treffer.push(bauen(start, dauer));
...
export { TYPEN, vorkommen, naechstes };
