...
// ------------------------------------------------- Betraege im Kalender

const mitBetrag = await routen.ruf('POST', '/api/eintrag', { text: 'Miete', datum: '2026-10-01', betrag: '-1.250,00 €' });
pruefe('ein Betrag in deutscher Schreibweise kommt als Zahl an', mitBetrag.eintrag.betrag, -1250);
await routen.ruf('POST', '/api/eintrag', { text: 'Gehalt', datum: '2026-10-28', betrag: '2100,5' });
await routen.ruf('POST', '/api/eintrag', { text: 'Kino', datum: '2026-10-12', datumBis: '2026-10-14', betrag: '-12,3' });
await routen.ruf('POST', '/api/eintrag', { text: 'ohne Geld', datum: '2026-10-05', betrag: '' });
await routen.ruf('POST', '/api/eintrag', {
  text: 'Abo', datum: '2026-08-15', betrag: '-9,99', wiederholung: { typ: 'monatlich', intervall: 1, bis: null }
});

const oktober = await routen.ruf('GET', '/api/monat?jahr=2026&monat=10');
pruefe('der Monat summiert seine Betraege', oktober.summe.saldo, 828.21);
pruefe('Einnahmen und Ausgaben getrennt', `${oktober.summe.plus}/${oktober.summe.minus}`, '2100.5/-1272.29');
pruefe('ein mehrtaegiger Eintrag zaehlt einmal, die Serie in jedem Monat', oktober.summe.anzahl, 4);
pruefe('ohne Betrag heisst ohne Betrag, nicht null Euro',
  oktober.tage['2026-10-05'].find((e) => e.text === 'ohne Geld').betrag, null);
pruefe('der Betrag steht am Vorkommen', oktober.tage['2026-10-01'].find((e) => e.text === 'Miete').betrag, -1250);

const september = await routen.ruf('GET', '/api/monat?jahr=2026&monat=9');
pruefe('im Monat davor zaehlt nur die Serie', `${september.summe.anzahl}/${september.summe.saldo}`, '1/-9.99');
const juli = await routen.ruf('GET', '/api/monat?jahr=2026&monat=7');
pruefe('vor dem Serienstart bleibt es leer', juli.summe.anzahl, 0);

// ------------------------------------------------- Ausnahmen in Serien

const serie = await routen.ruf('POST', '/api/eintrag', {
  text: 'Chor', datum: '2026-11-04', uhrzeit: '19:00',
  wiederholung: { typ: 'woechentlich', intervall: 1, bis: null, ausnahmen: ['2026-11-18', 'kein datum', '2026-11-18'] }
});
pruefe('Ausnahmen werden gesaeubert und sortiert', (serie.eintrag.wiederholung.ausnahmen || []).join(','), '2026-11-18');
const novemberTage = () => routen.ruf('GET', '/api/monat?jahr=2026&monat=11')
  // Das Gitter zeigt auch Randtage des Folgemonats — die bleiben hier aussen vor.
  .then((m) => Object.keys(m.tage).filter((t) => t.startsWith('2026-11') && m.tage[t].some((e) => e.text === 'Chor')).join(','));
pruefe('das ausgelassene Vorkommen fehlt, die Serie laeuft weiter',
  await novemberTage(), '2026-11-04,2026-11-11,2026-11-25');

const ausgelassen = await routen.ruf('POST', '/api/eintrag/ausnahme', { id: serie.eintrag.id, datum: '2026-11-25', wert: true });
pruefe('ein Vorkommen laesst sich mit einem Klick auslassen', ausgelassen.ausnahmen.join(','), '2026-11-18,2026-11-25');
pruefe('und ist danach weg', await novemberTage(), '2026-11-04,2026-11-11');
await routen.ruf('POST', '/api/eintrag/ausnahme', { id: serie.eintrag.id, datum: '2026-11-18', wert: false });
pruefe('zurueckgenommen kommt es wieder', await novemberTage(), '2026-11-04,2026-11-11,2026-11-18');
pruefe('die Serie selbst bleibt unangetastet',
  (await routen.ruf('GET', '/api/daten')).entries.find((e) => e.id === serie.eintrag.id).wiederholung.typ, 'woechentlich');
pruefe('ohne Serie gibt es keine Ausnahme',
  Boolean((await routen.ruf('POST', '/api/eintrag/ausnahme', { id: mitBetrag.eintrag.id, datum: '2026-10-01', wert: true })).fehler), true);
pruefe('naechstes Vorkommen ueberspringt die Ausnahme',
  wdh.naechstes({ datum: '2026-11-04', wiederholung: { typ: 'woechentlich', intervall: 1, ausnahmen: ['2026-11-11'] } }, '2026-11-05').start,
  '2026-11-18');

// ------------------------------------------------- Vorlagen

const zahnarzt = await routen.ruf('POST', '/api/vorlage', {
  name: '  Zahnarzt ', datum: '2026-10-01', datumBis: '2026-10-02', uhrzeit: '09:30', betrag: '-45,50',
  kategorie: 'kat_x', art: 'art_termin', prioritaet: 'hoch'
});
pruefe('eine Vorlage wird unter dem Namen gemerkt', zahnarzt.vorlage.name, 'Zahnarzt');
pruefe('mit Uhrzeit, Betrag, Kategorie, Art, Prioritaet und Dauer',
  [zahnarzt.vorlage.uhrzeit, zahnarzt.vorlage.betrag, zahnarzt.vorlage.kategorie, zahnarzt.vorlage.art, zahnarzt.vorlage.prioritaet, zahnarzt.vorlage.tage],
  ['09:30', -45.5, 'kat_x', 'art_termin', 'hoch', 1]);
pruefe('derselbe Name ersetzt die Vorlage statt sie zu doppeln',
  (await routen.ruf('POST', '/api/vorlage', { name: 'zahnarzt', uhrzeit: '10:00' })).ersetzt, true);
pruefe('und es bleibt bei einer', (await routen.ruf('GET', '/api/vorlagen')).vorlagen.filter((v) => v.name === 'zahnarzt').length, 1);

const vorgabe = await routen.ruf('GET', '/api/vorlage?name=ZAHNARZT&datum=2026-12-03');
pruefe('der Name fuellt die Felder nach, Gross- und Kleinschreibung egal', vorgabe.quelle, 'vorlage');
pruefe('die Dauer haengt am neuen Datum', vorgabe.vorlage.datumBis, null);
pruefe('die erneuerte Uhrzeit gilt', vorgabe.vorlage.uhrzeit, '10:00');

const messe = await routen.ruf('POST', '/api/vorlage', { name: 'Messe', datum: '2026-03-10', datumBis: '2026-03-12' });
pruefe('Mehrtaegiges merkt sich die Dauer', messe.vorlage.tage, 2);
pruefe('und rechnet sie am neuen Tag vor',
  (await routen.ruf('GET', '/api/vorlage?name=Messe&datum=2026-12-03')).vorlage.datumBis, '2026-12-05');

pruefe('ohne Vorlage springt der juengste Eintrag gleichen Namens ein',
  (await routen.ruf('GET', '/api/vorlage?name=chor&datum=2026-12-03')), { quelle: 'eintrag', vorlage: { uhrzeit: '19:00', adresse: null, betrag: null, kategorie: null, art: 'art_termin', prioritaet: 'mittel', tage: 0, datumBis: null } });
pruefe('ein unbekannter Name liefert nichts', (await routen.ruf('GET', '/api/vorlage?name=gibtsnicht')).vorlage, null);
const namen = (await routen.ruf('GET', '/api/vorlagen')).namen;
pruefe('die Namensliste beginnt mit den Vorlagen', namen.slice(0, 2), ['zahnarzt', 'Messe']);
pruefe('und enthaelt jeden Eintragstext genau einmal', namen.filter((n) => n === 'Chor').length, 1);
pruefe('ohne Namen keine Vorlage', Boolean((await routen.ruf('POST', '/api/vorlage', { uhrzeit: '08:00' })).fehler), true);

await routen.ruf('DELETE', `/api/vorlage?id=${messe.vorlage.id}`);
pruefe('geloescht ist sie weg', (await routen.ruf('GET', '/api/vorlage?name=Messe')).vorlage, null);
await routen.ruf('POST', '/api/kategorie', { name: 'Nebenbei', farbe: '#123456' });
pruefe('andere Schreibvorgaenge lassen die Vorlagen stehen',
  (await routen.ruf('GET', '/api/vorlagen')).vorlagen.map((v) => v.name).join(','), 'zahnarzt');

// ------------------------------------------------- Adresse am Eintrag

const mitAdresse = await routen.ruf('POST', '/api/eintrag', {
  text: 'Physio', datum: '2026-10-07', uhrzeit: '08:00', art: 'art_termin', adresse: '  Bahnhofstraße 3, 26789 Leer  '
});
pruefe('die Adresse wird getrimmt gespeichert', mitAdresse.eintrag.adresse, 'Bahnhofstraße 3, 26789 Leer');
pruefe('leer heisst keine Adresse',
  (await routen.ruf('POST', '/api/eintrag', { text: 'ohne', datum: '2026-10-07', adresse: '   ' })).eintrag.adresse, null);
const oktoberTag = (await routen.ruf('GET', '/api/monat?jahr=2026&monat=10')).tage['2026-10-07'];
pruefe('das Vorkommen traegt die Adresse in die Tagesliste', oktoberTag.find((v) => v.text === 'Physio').adresse, 'Bahnhofstraße 3, 26789 Leer');
pruefe('die Suche findet den Eintrag ueber die Adresse',
  suche('Bahnhofstraße', routen.daten(), 'alle').entries.map((e) => e.text).join(','), 'Physio');
const physioVorlage = await routen.ruf('POST', '/api/vorlage', { name: 'Physio', adresse: 'Bahnhofstraße 3, 26789 Leer', uhrzeit: '08:00' });
pruefe('die Vorlage merkt sich die Adresse', physioVorlage.vorlage.adresse, 'Bahnhofstraße 3, 26789 Leer');
const mailStand = mail.bauen({
  entries: [{ id: 'm1', datum: '2026-10-07', uhrzeit: '08:00', text: 'Physio', adresse: 'Bahnhofstraße 3', art: 'art_termin', prioritaet: 'mittel' }],
  tasks: [], gemeinden: [], kategorien: []
}, '2026-10-07');
pruefe('die Tagesmail nennt die Adresse', mailStand.text.includes('08:00 Physio, Bahnhofstraße 3'), true);
...
// ------------------------------------------------- Kalenderbetraege in den Finanzen

store.laden({
  'entries.json': {
    kategorien: [{ id: 'kat_wohnen', name: 'Wohnen & Nebenkosten', farbe: '#e0a86a' }, { id: 'kat_can', name: 'Can', farbe: '#3ddc84' }],
    entries: [
      { id: 'k1', datum: '2026-07-01', text: 'Miete', betrag: -800, kategorie: 'kat_wohnen', prioritaet: 'mittel',
        wiederholung: { typ: 'monatlich', intervall: 1, bis: null } },
      { id: 'k2', datum: '2026-07-20', text: 'Honorar', betrag: 350.5, kategorie: 'kat_can', prioritaet: 'mittel' },
      { id: 'k3', datum: '2026-07-21', text: 'ohne Betrag', prioritaet: 'mittel' },
      { id: 'k4', datum: '2026-08-02', text: 'Zahnarzt', betrag: -45, prioritaet: 'mittel' }
    ]
  },
  'finanzen.json': {
    buchungen: [{ id: 'f1', datum: '2026-07-03', text: 'a', betrag: 100, bank: 'N26', kategorie: null }],
    kategorien: [{ id: 'fk_wohnen', name: 'Wohnen & Nebenkosten', farbe: '#e0a86a' }], regeln: [], importe: []
  }
});

const juliFin = fin.auswertung({ von: '2026-07-01', bis: '2026-07-31' });
pruefe('Kalenderbetraege stehen als Buchungen in den Finanzen',
  juliFin.buchungen.filter((b) => b.quelle === 'kalender').map((b) => `${b.datum} ${b.text} ${b.betrag}`).sort(),
  ['2026-07-01 Miete -800', '2026-07-20 Honorar 350.5']);
pruefe('sie zaehlen in Einnahmen und Ausgaben mit', [juliFin.einnahmen, juliFin.ausgaben, juliFin.saldo], [450.5, 800, -349.5]);
pruefe('und werden gesondert gezaehlt', [juliFin.anzahl, juliFin.ausKalender], [3, 2]);
pruefe('eine gleichnamige Finanzkategorie wird uebernommen, sonst keine',
  juliFin.buchungen.filter((b) => b.quelle === 'kalender').map((b) => b.kategorie).sort(), [null, 'fk_wohnen'].sort());
pruefe('eine monatliche Serie zaehlt in jedem Monat',
  fin.auswertung({ von: '2026-07-01', bis: '2026-09-30', bank: 'Kalender' }).buchungen.filter((b) => b.text === 'Miete').length, 3);
pruefe('ohne Zeitraum reicht der Kalender bis Jahresende, nicht weiter',
  fin.auswertung({ bank: 'Kalender' }).buchungen.filter((b) => b.text === 'Miete').map((b) => b.datum).sort().pop(), `${store.heute().slice(0, 4)}-12-01`);

const fkKal = fin.kachel('2026-07-15');
pruefe('die Startseite fuehrt den Kalender als eigenes Konto', fkKal.konten.map((k) => k.bank), ['N26', 'Kalender']);
pruefe('mit den Monatssummen aus dem Kalender', [fkKal.konten[1].einnahmen, fkKal.konten[1].ausgaben], [350.5, 800]);
pruefe('die Einzelsalden ergeben auch so den Gesamtsaldo',
  Math.round(fkKal.konten.reduce((s, k) => s + k.saldo, 0) * 100) / 100, fkKal.saldo);
pruefe('Kalenderbuchungen zaehlen nicht als "ohne Kategorie" der Auszuege', fkKal.ohneKategorie, 1);
pruefe('eine Kalenderbuchung laesst sich in den Finanzen nicht umbiegen',
  Boolean(fin.buchungAendern({ id: 'kal_k1_2026-07-01', kategorie: 'fk_wohnen' }).fehler), true);
...
pruefe('die letzte Silbe steht unter der letzten der Zeile darueber', sicht.gruppen[0].zeilen[1].versatz, 0);
pruefe('auch eine kurze Zeile endet buendig', sicht.gruppen[0].zeilen[2].versatz, 3);
...
const BEREICHE_TEST = ['reime', 'zeilen', 'texte'];
reimedaten.kategorieSpeichern({ name: 'Battle', stichwoerter: 'steuer, amt' });
pruefe('Kategorie angelegt', reimedaten.lesen().kategorien.length, 1);
pruefe('mit gesaeuberten Stichwoertern', reimedaten.lesen().kategorien[0].stichwoerter.join('|'), 'steuer|amt');
pruefe('und sie steht in jedem der drei Reiter',
  BEREICHE_TEST.map((b) => reimedaten.ansicht(b, null).kategorien.length).join(','), '1,1,1');
...
store.write('reime.json', structuredClone(store.defaults['reime.json']));
reimedaten.kategorieSpeichern({ name: 'Vergleiche', stichwoerter: 'wie' });
const umzugKat = reimedaten.lesen().kategorien[0].id;
...
const umzug = reimedaten.kategorieKopieren({ von: 'reime', nach: 'zeilen', id: umzugKat });
pruefe('die Kategorie selbst bleibt stehen, sie gilt ueberall',
  reimedaten.lesen().kategorien.map((k) => k.name).join(','), 'Vergleiche');
...
pruefe('die Vorlage bleibt stehen, abgeschrieben wird nur',
  reimedaten.lesen().gruppen.map((g) => g.kopf).join(','), 'ohne Kategorie,Wüstenfuchs');

const umzugZeile = reimedaten.lesen().zeilen[1].id;
reimedaten.eintragKopieren({ von: 'zeilen', nach: 'reime', id: umzugZeile });
pruefe('eine einzelne Zeile wird zum Kopfbegriff', reimedaten.lesen().gruppen[0].kopf, 'Überschuss');
pruefe('die Kategorie kommt mit, sie gilt auch im Ziel', reimedaten.lesen().gruppen[0].kategorien[0], umzugKat);
pruefe('und die Zeile steht weiterhin da', reimedaten.lesen().zeilen.length, 2);
pruefe('eine Abschrift in den eigenen Reiter wird abgelehnt',
  Boolean(reimedaten.eintragKopieren({ von: 'zeilen', nach: 'zeilen', id: umzugZeile }).fehler), true);

reimedaten.textSpeichern({ titel: 'Strophe', inhalt: 'erste Zeile\nzweite Zeile' });
reimedaten.eintragKopieren({ von: 'texte', nach: 'zeilen', id: reimedaten.lesen().texte[0].id });
...
pruefe('und bleibt daneben bestehen', reimedaten.lesen().texte.length, 1);
...
pruefe('ohne Schub gilt die gerechnete Ausrichtung', schubStand()[1].versatz, 0);
reimedaten.schubSetzen({ id: schubReim, schritt: 1 });
pruefe('ein Schritt nach rechts', schubStand()[1].versatz, 1);
...
pruefe('die Gruppe rueckt nach, statt aus dem Raster zu fallen', schubStand()[0].versatz, 3);
...
pruefe('zuruecksetzen stellt die Rechnung wieder her', schubStand()[1].versatz, 0);
...
const zeilenStand = () => reimedaten.ansicht('zeilen', null).zeilen.map((z) => z.teile[0]);
...
// ---- Woerter innerhalb einer Zeile schieben

reimedaten.eintragSpeichern({ gruppe: schubGruppe, text: 'neue Feier' });
const wortReim = reimedaten.lesen().gruppen[0].eintraege
  .find((e) => e.text === 'neue Feier').id;
const wortZeile = () => reimedaten.ansicht('reime', null).gruppen[0].zeilen
  .find((z) => z.id === wortReim);

reimedaten.schubSetzen({ id: wortReim, wort: 1, schritt: 2 });
pruefe('ein Wort laesst sich innerhalb der Zeile schieben',
  reimedaten.lesen().gruppen[0].eintraege.find((e) => e.id === wortReim).wortSchub['1'], 2);
pruefe('die Luecke kommt in der Ansicht an', wortZeile().wortSchub['1'], 2);

reimedaten.schubSetzen({ id: wortReim, wort: 1, schritt: -5 });
pruefe('vor das Wort davor geht es nicht',
  'wortSchub' in reimedaten.lesen().gruppen[0].eintraege.find((e) => e.id === wortReim), false);

reimedaten.schubSetzen({ id: wortReim, wort: 0, schritt: 3 });
pruefe('das erste Wort schiebt die ganze Zeile',
  reimedaten.lesen().gruppen[0].eintraege.find((e) => e.id === wortReim).schub, 3);

reimedaten.schubSetzen({ id: wortReim, wort: 1, schritt: 1 });
reimedaten.schubSetzen({ id: wortReim, zuruecksetzen: true });
pruefe('zuruecksetzen raeumt auch die Luecken weg',
  JSON.stringify(wortZeile().wortSchub), '{}');

// ---- Klammern: gesagt, aber nicht gezaehlt

pruefe('io, iu und ui stehen als Laute bereit',
  ['io', 'iu', 'ui'].every((l) => silben.LAUTE.includes(l) && reim.FARBEN[l]), true);
pruefe('aber nicht aus der Schreibweise: Nation bleibt zweisilbig',
  silben.kerne('Nation').join(','), 'a,i,o');
pruefe('als Anmerkung werden sie angenommen',
  reim.wortNormalisieren({ silben: [{ anmerkungen: [{ laut: 'ui', prioritaet: 'hoch' }] }] }).silben[0].anmerkungen.length, 1);
pruefe('und als Korrektur',
  reim.wortNormalisieren({ silben: [{ korrektur: 'io' }] }).silben[0].korrektur, 'io');

pruefe('was in Klammern steht, ist als solches erkannt',
  silben.kette('nur (kleine Welt)').map((x) => (x.klammer ? 1 : 0)).join(''), '0111');
pruefe('die Klammer selbst gehoert schon hinein',
  silben.klammertiefe('a(b)c').join(''), '01110');

const stumm = K('nur (kleine Welt)');
pruefe('Eingeklammertes bleibt im Raster stehen', stumm.length, 4);
pruefe('zaehlt aber nicht mit', reim.zaehlung(stumm, null, null).gesamt, 1);
pruefe('und wird auch nicht gesucht', reim.suchen(['ei'], stumm, {}), null);

const eintrag = (zaehlung) => ({ korrektur: null, relevanz: 'buchstabe', zaehlung, anmerkungen: [] });

/* Der Fall aus der Vorgabe: mitten in der Klammer soll wieder etwas zaehlen.
   Dann liegt zwischen den zaehlenden Silben eine Luecke, und die soll bleiben. */
const mitLuecke = K('nur (kleine Welt)', { welt: { silben: [eintrag('mit')] } });
pruefe('eine angemerkte Silbe zaehlt trotz Klammer',
  reim.zaehlung(mitLuecke, null, null).gesamt, 2);
pruefe('die Luecke dazwischen bleibt bestehen',
  reim.zaehlende(mitLuecke).map((x) => x.stelle).join(','), '0,3');
pruefe('und die Suche findet die freigegebene Silbe',
  reim.suchen(['e'], mitLuecke, {}).von, 3);

const gestrichen = K('nur (kleine Welt)', { welt: { silben: [eintrag('weg')] } });
pruefe('eine gestrichene Silbe faellt ganz aus', gestrichen.length, 3);
pruefe('und hinterlaesst keine Luecke',
  gestrichen.map((x) => x.kern).join(','), 'u,ei,e');

pruefe('eine Anmerkung wiegt von selbst schwerer',
  reim.wortNormalisieren({ silben: [{ anmerkungen: [{ laut: 'a', prioritaet: 'mittel' }] }] })
    .silben[0].relevanz, 'anmerkung');
pruefe('ohne Anmerkung bleibt es beim Buchstaben',
  reim.wortNormalisieren({ silben: [{ anmerkungen: [] }] }).silben[0].relevanz, 'buchstabe');

store.write('reime.json', structuredClone(store.defaults['reime.json']));
reimedaten.gruppeSpeichern({ kopf: 'Hundesteuer' });
const klammerGruppe = reimedaten.lesen().gruppen[0].id;
reimedaten.eintragSpeichern({ gruppe: klammerGruppe, text: '(oh) Mäuseplage' });
const klammerSicht = () => reimedaten.ansicht('reime', null).gruppen[0].zeilen;
pruefe('die Endregel rechnet in Rasterspalten, nicht in Zaehlsilben',
  klammerSicht()[1].versatz, 0);
pruefe('und die Gruppe rueckt dafuer nach', klammerSicht()[0].versatz, 1);
pruefe('der Zaehler laesst das Eingeklammerte aus',
  klammerSicht()[1].zaehlung.gesamt, 4);

pruefe('eine einsilbige Silbe laesst sich nicht streichen', Boolean(reimedaten.wortSpeichern({
  wort: 'oh', silbeIndex: 0, korrektur: null, relevanz: 'buchstabe',
  zaehlung: 'weg', anmerkungen: []
}).fehler), true);

reimedaten.wortSpeichern({
  wort: 'Mäuseplage', silbeIndex: 3, korrektur: null, relevanz: 'buchstabe',
  zaehlung: 'weg', anmerkungen: []
});
pruefe('gestrichene Silben verschwinden auch im Bestand',
  klammerSicht()[1].silben.length, 4);
pruefe('die uebrigen Silben des Wortes wissen davon',
  klammerSicht()[1].silben[1].wortWeg.map((w) => w.kern).join(','), 'e');

reimedaten.wortSpeichern({
  wort: 'Mäuseplage', silbeIndex: 3, nurZaehlung: true, zaehlung: null
});
pruefe('und kommen zurueck, sobald die Anmerkung faellt',
  klammerSicht()[1].silben.length, 5);
pruefe('ohne Anmerkung bleibt keine Karteileiche',
  'mäuseplage' in reimedaten.lesen().woerter, false);

// ---- Kategorien gelten ueberall

store.write('reime.json', structuredClone(store.defaults['reime.json']));

/* Wer die App schon benutzt hat, hat drei getrennte Listen in der Ablage. Die
   werden beim Lesen einmal zusammengelegt, ohne dass etwas verlorengeht. */
store.write('reime.json', {
  ...structuredClone(store.defaults['reime.json']),
  kategorien: {
    reime: [{ id: 'k1', name: 'Battle', farbe: '#ff0000', stichwoerter: [] }],
    zeilen: [{ id: 'k2', name: 'Punchlines', farbe: '#00ff00', stichwoerter: [] }],
    texte: [{ id: 'k3', name: 'Strophen', farbe: '#0000ff', stichwoerter: [] }]
  }
});
pruefe('alte Listen je Reiter werden zu einer',
  reimedaten.lesen().kategorien.map((k) => k.name).join(','), 'Battle,Punchlines,Strophen');

reimedaten.gruppeSpeichern({ kopf: 'Hundesteuer', kategorien: ['k1'] });
reimedaten.zeileSpeichern({ text: 'eine Zeile', kategorien: ['k1'] });
reimedaten.textSpeichern({ titel: 'ein Text', inhalt: 'nur eine Zeile', kategorien: ['k1'] });
const battle = () => reimedaten.ansicht('reime', null).kategorien.find((k) => k.id === 'k1');
pruefe('die Leiste sagt, wo etwas unter der Kategorie liegt',
  `${battle().anzahl.reime}/${battle().anzahl.zeilen}/${battle().anzahl.texte}`, '1/1/1');
pruefe('und der Filter greift in jedem Reiter',
  reimedaten.ansicht('texte', 'k1').texte.length + reimedaten.ansicht('texte', 'k2').texte.length, 1);
pruefe('was benutzt wird, laesst sich nicht loeschen',
  Boolean(reimedaten.kategorieLoeschen('k1').fehler), true);
pruefe('was nirgends haengt, schon', reimedaten.kategorieLoeschen('k3').ok, true);

// ---- Nach oben und unten schieben

store.write('reime.json', structuredClone(store.defaults['reime.json']));
reimedaten.kategorieSpeichern({ name: 'Nur diese' });
const nurDiese = reimedaten.lesen().kategorien[0].id;
for (const kopf of ['erste', 'zweite', 'dritte']) reimedaten.gruppeSpeichern({ kopf });
const koepfe = () => reimedaten.lesen().gruppen.map((g) => g.kopf).join(',');
pruefe('neue Gruppen stehen oben', koepfe(), 'dritte,zweite,erste');

const dritte = reimedaten.lesen().gruppen[0].id;
reimedaten.ordnen({ id: dritte, schritt: 1 });
pruefe('eine Gruppe geht nach unten', koepfe(), 'zweite,dritte,erste');
reimedaten.ordnen({ id: dritte, schritt: -1 });
pruefe('und wieder nach oben', koepfe(), 'dritte,zweite,erste');
pruefe('ganz oben ist Schluss', Boolean(reimedaten.ordnen({ id: dritte, schritt: -1 }).fehler), true);

reimedaten.gruppeSpeichern({ id: dritte, kopf: 'dritte', kategorien: [nurDiese] });
reimedaten.gruppeSpeichern({ id: reimedaten.lesen().gruppen[2].id, kopf: 'erste', kategorien: [nurDiese] });
reimedaten.ordnen({ id: dritte, schritt: 1, kategorie: nurDiese });
pruefe('unter einem Filter springt der Schritt ueber Unsichtbares', koepfe(), 'zweite,erste,dritte');

reimedaten.eintragSpeichern({ gruppe: dritte, text: 'Reim eins' });
reimedaten.eintragSpeichern({ gruppe: dritte, text: 'Reim zwei' });
const reime = () => reimedaten.lesen().gruppen.find((g) => g.id === dritte).eintraege.map((e) => e.text).join(',');
reimedaten.ordnen({ id: reimedaten.lesen().gruppen.find((g) => g.id === dritte).eintraege[1].id, schritt: -1 });
pruefe('auch ein einzelner Reim laesst sich schieben', reime(), 'Reim zwei,Reim eins');

reimedaten.textSpeichern({ titel: 'Strophe', inhalt: 'A\nB\nC' });
const textId = reimedaten.lesen().texte[0].id;
const textZeilen = () => reimedaten.lesen().texte[0].zeilen.map((z) => z.text).join('');
const ersteZeile = reimedaten.lesen().texte[0].zeilen[0].id;
reimedaten.ordnen({ id: ersteZeile, schritt: 1 });
pruefe('und eine Zeile innerhalb eines Textes', textZeilen(), 'BAC');

// ---- Textzeilen behalten ihre Verschiebung

reimedaten.schubSetzen({ id: ersteZeile, schritt: 2 });
reimedaten.textSpeichern({ id: textId, titel: 'Strophe', inhalt: 'B\nA\nC neu' });
const nachher = reimedaten.lesen().texte[0].zeilen;
pruefe('unveraenderte Zeilen behalten Kennung und Schub',
  `${nachher[1].id === ersteZeile}/${nachher[1].schub}`, 'true/2');
pruefe('eine geaenderte Zeile faengt neu an', nachher[2].schub === undefined, true);

// ---- Die letzte Silbe steht unter der letzten

store.write('reime.json', structuredClone(store.defaults['reime.json']));
reimedaten.gruppeSpeichern({ kopf: 'Hundesteuer' });
const endGruppe = reimedaten.lesen().gruppen[0].id;
reimedaten.eintragSpeichern({ gruppe: endGruppe, text: 'Ohr' });
reimedaten.eintragSpeichern({ gruppe: endGruppe, text: 'ein langer Abenteuerspielplatz' });
const endStand = () => reimedaten.ansicht('reime', null).gruppen[0].zeilen;

/* Buendig heisst: Versatz plus Silbenzahl ist ueberall gleich. Damit haengt der
   Test nicht daran, wie weit die Gruppe insgesamt nach rechts gerueckt ist. */
const endKante = (z) => z.versatz + z.silben.length;
pruefe('alle Zeilen enden in derselben Spalte',
  new Set(endStand().map(endKante)).size, 1);

pruefe('die laengste Zeile faengt ganz links an',
  Math.min(...endStand().map((z) => z.versatz)), 0);

// Auch was laengst dasteht, richtet sich am Ende aus — die Regel gilt immer.
const alteDaten = reimedaten.lesen();
alteDaten.gruppen[0].eintraege.push({ id: 'alt1', text: 'Mäuseplage' });
store.write('reime.json', alteDaten);
pruefe('auch ein alter Eintrag endet buendig',
  endKante(endStand()[3]), endKante(endStand()[0]));

/* Was in Klammern hinterherlaeuft, gehoert nicht zum Reim: die Endregel misst
   an der letzten mitzaehlenden Silbe, nicht an der letzten Zelle. */
reimedaten.eintragSpeichern({ gruppe: endGruppe, text: 'Hundesteuer (na gut)' });
const mitKlammer = endStand()[4];
pruefe('eine Klammer am Ende zieht die Zeile nicht mit',
  mitKlammer.versatz + 4, endKante(endStand()[0]));

// ---- Auch in Zeilen und Texten

store.write('reime.json', structuredClone(store.defaults['reime.json']));
reimedaten.textSpeichern({ titel: 'Strophe', inhalt: 'Hundesteuer\nOhr\n\nMäuseplage' });
const endText = () => reimedaten.ansicht('texte', null).texte[0].zeilen;
pruefe('auch die Zeilen eines Textes enden buendig',
  new Set(endText().filter((z) => z.silben.length).map(endKante)).size, 1);
pruefe('eine Leerzeile bleibt aussen vor', endText()[2].versatz, 0);

reimedaten.zeileSpeichern({ text: 'Hundesteuer' });
reimedaten.zeileSpeichern({ text: 'Ohr' });
const endZeilen = reimedaten.ansicht('zeilen', null).zeilen.map((z) => z.teile[0]);
pruefe('und einzelne Zeilen ebenso',
  new Set(endZeilen.map(endKante)).size, 1);

// ---- Absaetze im Reiter „Zeilen"

store.write('reime.json', structuredClone(store.defaults['reime.json']));
reimedaten.zeileSpeichern({ text: 'Hundesteuer\nOhr\nMäuseplage' });
const absatz = () => reimedaten.ansicht('zeilen', null).zeilen[0];
pruefe('ein Absatz bleibt ein Eintrag', reimedaten.ansicht('zeilen', null).zeilen.length, 1);
pruefe('mit allen Zeilen darin', absatz().teile.map((t) => t.text).join('|'), 'Hundesteuer|Ohr|Mäuseplage');
pruefe('zum Aendern kommt der ganze Absatz zurueck', absatz().inhalt.split('\n').length, 3);
pruefe('und auch die Zeilen eines Absatzes enden buendig',
  new Set(absatz().teile.map(endKante)).size, 1);

const zweiteTeil = absatz().teile[1].id;
reimedaten.schubSetzen({ id: zweiteTeil, schritt: 2 });
pruefe('eine einzelne Zeile des Absatzes laesst sich schieben', absatz().teile[1].schub, 2);
reimedaten.zeileSpeichern({ id: absatz().id, text: 'Hundesteuer\nOhr\nAbenteuer' });
pruefe('die unveraenderte Zeile behaelt ihre Verschiebung', absatz().teile[1].schub, 2);
pruefe('die geaenderte faengt neu an', absatz().teile[2].schub, 0);
pruefe('der Eintrag behaelt seine Kennung', reimedaten.ansicht('zeilen', null).zeilen[0].id, absatz().id);

reimedaten.eintragKopieren({ von: 'zeilen', nach: 'reime', id: absatz().id });
pruefe('abgeschrieben wird der ganze Absatz',
  [reimedaten.lesen().gruppen[0].kopf, ...reimedaten.lesen().gruppen[0].eintraege.map((e) => e.text)].join('|'),
  'Hundesteuer|Ohr|Abenteuer');
pruefe('und der Absatz steht unveraendert weiter da',
  reimedaten.ansicht('zeilen', null).zeilen[0].teile.length, 3);
...
/* Der Grund fuer diesen Test: die Module unter lib/ laedt app.js selbst nach
   und sie trugen deshalb keine Fassungskennung. Ein Browser hat dann stunden-
   lang eine alte routen.js benutzt, waehrend app.js schon neu war — sichtbar
   als "Unbekannter Endpunkt" fuer eine laengst gebaute Route. Die Importkarte
   in index.html haengt die Kennung an jeden Modulpfad; hier wird geprueft,
   dass sie vollstaendig und aktuell ist. */

const wurzel = new URL('..', import.meta.url);
const auslieferung = fs.readFileSync(new URL('index.html', wurzel), 'utf8');
const karte = JSON.parse((auslieferung.match(/<script type="importmap">([\s\S]*?)<\/script>/) || [])[1] || '{}');
const module = fs.readdirSync(new URL('lib/', wurzel))
  .filter((d) => d.endsWith('.js') && d !== 'selbsttest.js');

pruefe('jedes Modul steht in der Importkarte',
  module.filter((d) => !karte.imports || !karte.imports[`./lib/${d}`]).join(','), '');
pruefe('und zeigt auf die laufende Fassung',
  Object.values(karte.imports || {}).filter((z) => !z.endsWith(`?v=${FASSUNG}`)).join(','), '');
pruefe('index.html laedt app.js mit derselben Kennung',
  auslieferung.includes(`./app.js?v=${FASSUNG}`), true);
pruefe('und style.css ebenso', auslieferung.includes(`style.css?v=${FASSUNG}`), true);
