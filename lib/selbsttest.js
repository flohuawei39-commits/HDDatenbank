...
/* Prueft die Teile, bei denen ein Fehler still bleibt: Datumsrechnung,
   Serien, Schnelleingabe und Tagesmail. Aufruf: node lib/selbsttest.js */

// Eigener Datenordner, BEVOR store geladen wird: der Test darf den Echtbestand nie anfassen.
process.env.HDD_BASIS = require('path').join(require('os').tmpdir(), 'hdd-selbsttest');

const D = require('./datum');
const wdh = require('./recurrence');
const quick = require('./quickparse');
const { suche } = require('./search');
...
const configSetzen = (m) => store.write('config.json', { pinHash: 'x', mail: m });
...
const { sichererName } = require('./gemeinden');
pruefe('Pfadwechsel wird entschaerft', sichererName('../../etc/passwd'), '__.._etc_passwd');
pruefe('kein Schraegstrich ueberlebt', /[\\/]/.test(sichererName('../../etc/passwd')), false);
pruefe('Windows-Sonderzeichen fallen weg', sichererName('Antrag: Förder*plan?.pdf'), 'Antrag_ Förder_plan_.pdf');
pruefe('Leerzeichen und Bindestriche bleiben lesbar', sichererName('Brief an Gemeinde 2026-07-25.pdf'), 'Brief an Gemeinde 2026-07-25.pdf');
pruefe('fuehrende Punkte werden ersetzt', sichererName('.htaccess'), '_htaccess');
pruefe('Steuerzeichen werden entfernt', sichererName(`Antrag${String.fromCharCode(0)}${String.fromCharCode(9)}.pdf`), 'Antrag.pdf');
pruefe('leerer Name bekommt einen Ersatz', sichererName(''), 'datei');
