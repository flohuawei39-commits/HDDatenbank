...
const kennung = (bank, konto, buchung, lauf = 0) => {
  ...
  return crypto.createHash('sha256').update(roh).digest('hex').slice(0, 16);
};
 ...
 */
const auswertung = ({ von = null, bis = null, bereich = null } = {}) => {
  const daten = lesen();
...
/** Kompakte Kachel für die Startseite: laufender Monat. */
const kachel = (heute) => {
  const monat = heute.slice(0, 7);
  const a = auswertung({ von: `${monat}-01`, bis: `${monat}-31` });
  const daten = lesen();
  return {
    monat,
...
module.exports = {
  BEREICHE, START_KATEGORIEN, lesen, vorschau, uebernehmen, buchungAendern,
