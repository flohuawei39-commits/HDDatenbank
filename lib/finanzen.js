...
import * as kalender from './kalendergeld.js';
...
const buchungAendern = (koerper) => {
  if (String(koerper.id || '').startsWith('kal_')) return { fehler: 'Kalenderbeträge werden im Kalender geändert.' };
  const daten = lesen();
...
const auswertung = ({ von = null, bis = null, bereich = null, bank = null } = {}) => {
  const daten = lesen();
  // Kalenderbetraege laufen als eigenes Konto "Kalender" mit — siehe kalendergeld.js.
  const ausKalender = kalender.buchungen(von, bis, daten.kategorien);
  const passend = [...daten.buchungen, ...ausKalender].filter((b) => {
  ...
  return {
    ...
    ausKalender: passend.filter((b) => b.quelle === 'kalender').length,
...
const kachel = (heute) => {
  ...
  // Bankkonten alphabetisch, dann der Kalender, zuletzt Sonstige.
  const rang = (name) => (name === OHNE_BANK ? 2 : name === kalender.BANK ? 1 : 0);
  const namen = [...new Set(
    [...daten.buchungen, ...ausKalender]
      .filter((b) => b.datum >= von && b.datum <= bis)
      .map((b) => b.bank || OHNE_BANK)
  )].sort((x, y) => rang(x) - rang(y) || x.localeCompare(y));
  ...
  return {
    ...
    anzahl: a.anzahl,
    konten,
    ausKalender: ausKalender.length,
    ohneKategorie: daten.buchungen.filter((b) => !b.kategorie && !b.umbuchung).length,
    hatDaten: daten.buchungen.length > 0 || ausKalender.length > 0
  };
};
