...
/* Wie eine Silbe in die Zaehlung eingeht:
     mit  — sie zaehlt, der Normalfall ausserhalb von Klammern;
     ohne — sie steht da, zaehlt aber nicht mit und behaelt ihren Platz. Genau
            so entsteht die Luecke, wenn mitten in einer Klammer wieder etwas
            zaehlen soll;
     weg  — sie faellt ganz aus, ohne Platz und ohne Luecke. Fuer Silben, die
            man gar nicht hoert, etwa bei englischer Aussprache.             */
const ZAEHLUNG = ['mit', 'ohne', 'weg'];
...
const FARBEN = {
  ...
  ä: '#e963a1',
  // Die drei Doppelklaenge mit i liegen absichtlich nicht neben i und ie.
  io: '#d1a05a', iu: '#5ad1b8', ui: '#a55ad1'
};
  ...
  return {
    silben: silbenListe.map((s) => {
      ...
      return {
        ...
        // Wer eine Anmerkung setzt, meint sie auch: ohne ausdrueckliche Wahl
        // wiegt sie ab jetzt schwerer als der geschriebene Buchstabe.
        relevanz: RELEVANZ.includes(eintrag.relevanz)
          ? eintrag.relevanz
          : (anmerkungen.length ? 'anmerkung' : 'buchstabe'),
        // null heisst: richtet sich nach den Klammern im Text.
        zaehlung: ZAEHLUNG.includes(eintrag.zaehlung) ? eintrag.zaehlung : null,
        anmerkungen
      };
    })
  };
...
const silbeAnreichern = (silbe, wortDaten) => {
  ...
  /* Die Klammer im Text gibt die Vorgabe, die Anmerkung am Wort sticht sie. So
     bleiben vierzehn eingeklammerte Silben stumm, waehrend die angemerkten
     sechs bis neun mittendrin wieder mitzaehlen. */
  const zaehlWeise = (zusatz && zusatz.zaehlung) || (silbe.klammer ? 'ohne' : 'mit');
  ...
  return {
    ...
    kern,
    zaehlWeise,
    zaehlt: zaehlWeise === 'mit',
    entfaellt: zaehlWeise === 'weg',
    klammer: Boolean(silbe.klammer),
 ...
 */
const kette = (text, ablage = {}) => {
  const alle = silben.kette(text).map((s) => silbeAnreichern(s, ablage[s.schluessel] || null));

  /* Gestrichene Silben haben keine Zelle und damit auch nichts zum Anklicken.
     Damit man sie zurueckholen kann, merken sich die uebrigen Silben desselben
     Wortes, welche fehlen — das Silbenblatt bietet sie dort wieder an. */
  const fehlend = new Map();
  for (const s of alle) {
    if (!s.entfaellt) continue;
    const liste = fehlend.get(s.wortIndex) || [];
    liste.push({ silbeIndex: s.silbeIndex, kern: s.kern });
    fehlend.set(s.wortIndex, liste);
  }

  return alle
    .filter((s) => !s.entfaellt)
    .map((s) => ({ ...s, wortWeg: fehlend.get(s.wortIndex) || [] }));
};
...
/**
 * Die zaehlenden Silben einer Kette, jede mit der Stelle, an der sie im Raster
 * steht. Verglichen, gesucht und gezaehlt wird nur auf ihnen; gezeichnet wird
 * trotzdem die ganze Kette, damit die Luecken sichtbar bleiben.
 */
const zaehlende = (kette) => kette
  .map((s, stelle) => ({ s, stelle }))
  .filter((x) => x.s.zaehlt !== false);

/**
 * Laengste ununterbrochene Uebereinstimmung zweier Ketten.
 *
 * Gezaehlt wird in zaehlenden Silben, zurueckgegeben werden die Stellen im
 * Raster: die Ausrichtung im Reime-Reiter schiebt die Zeilen genau daran
 * zurecht, und die kennt nur Rasterspalten.
 */
const vergleich = (a, b) => {
  const A = zaehlende(a);
  const B = zaehlende(b);
  ...
  for (let i = 0; i < A.length; i += 1) {
    for (let j = 0; j < B.length; j += 1) {
      let l = 0;
      let g = 0;
      while (i + l < A.length && j + l < B.length && passt(A[i + l].s, B[j + l].s)) {
        g += gewicht(A[i + l].s, B[j + l].s);
        l += 1;
      }
      if (l > bestes.laenge || (l === bestes.laenge && l > 0 && g > bestes.gewicht)) {
        bestes = { laenge: l, a: A[i].stelle, b: B[j].stelle, gewicht: g };
      }
    }
  }
  return bestes;
...
const suchen = (muster, kette, wahl = {}) => {
  ...
  const m = (Array.isArray(muster) ? muster : [])
    ...
    .slice(0, MAX_MUSTER);
  // Eingeklammertes wird nicht gesucht: es zaehlt auch sonst nirgends mit.
  const gezaehlt = zaehlende(kette);
  if (!m.length || !gezaehlt.length) return null;

  const trifft = (eintrag, laut) => eintrag.s.laute.includes(laut);

  // Startstellen: jede Silbe, die die erste Mustersilbe traegt.
  let stand = new Map();
  for (let i = 0; i < gezaehlt.length; i += 1) {
    if (trifft(gezaehlt[i], m[0])) stand.set(i, { verbrauch: 0, stellen: [i] });
  }

  for (let j = 1; j < m.length; j += 1) {
    const naechster = new Map();
    for (const [pos, wert] of stand) {
      const grenze = Math.min(gezaehlt.length - 1, pos + 1 + (budget - wert.verbrauch));
      for (let p = pos + 1; p <= grenze; p += 1) {
        if (!trifft(gezaehlt[p], m[j])) continue;
  ...
  let bestes = null;
  for (const [pos, wert] of stand) {
    if (nurEnde && pos !== gezaehlt.length - 1) continue;
    if (!bestes || wert.verbrauch < bestes.verbrauch) bestes = wert;
  }
  if (!bestes) return null;

  // Zurueck in Rasterstellen, damit die Oberflaeche die richtigen Zellen hebt.
  const stellen = bestes.stellen.map((i) => gezaehlt[i].stelle);
  return {
    stellen,
    verbrauch: bestes.verbrauch,
    von: stellen[0],
    bis: stellen[stellen.length - 1]
  };
};
...
const zaehlung = (eigene, oben, unten) => ({
  ...
  gesamt: zaehlende(eigene).length
});

export {
  PRIORITAETEN, PRIO_GEWICHT, RELEVANZ, ZAEHLUNG, MAX_ANMERKUNGEN, MAX_MUSTER, FARBEN,
  wortNormalisieren, silbeAnreichern, kette, passt, zaehlende, vergleich, suchen,
  vorschlaege, zaehlung
};
