/* pdfjs wird einmal geladen und gemerkt. Im Browser liegt es unter /vendor/,
   im Selbsttest unter Node im node_modules-Ordner. Ausser diesem Ladeweg ist
   an dieser Datei nichts umgebungsabhaengig. */
let pdfjsCache = null;

const imBrowser = typeof window !== 'undefined' && typeof document !== 'undefined';

const pdfjsLaden = async () => {
  if (pdfjsCache) return pdfjsCache;

  if (imBrowser) {
    pdfjsCache = await import('../vendor/pdf.mjs');
    // Ohne eigenen Arbeiter zerlegt pdfjs im Hauptstrang und blockiert die Oberflaeche.
    pdfjsCache.GlobalWorkerOptions.workerSrc = new URL('../vendor/pdf.worker.mjs', import.meta.url).href;
    return pdfjsCache;
  }
  ...
  const ziel = path.join(hier, '..', 'node_modules', 'pdfjs-dist', 'legacy', 'build', 'pdf.mjs');
...
export { lesen, zuZeilen, zahl, betragInSpalte, linksVon, deutschesDatum, punktDatum, Y_TOLERANZ };
