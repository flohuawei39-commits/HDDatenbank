...
/* Betraege kommen in deutscher Schreibweise ("12,50", "-8", "1.234,00 €") und
   liegen auf Cent gerundet in der Ablage. Leer heisst: kein Betrag, und das
   ist etwas anderes als null Euro. */
const betragOderNull = (wert) => {
...
const wiederholungPruefen = (wert) => {
  ...
  // Ausnahmen sind Starttage einzelner Vorkommen; doppelte und krumme fliegen raus.
  const ausnahmen = [...new Set((Array.isArray(wert.ausnahmen) ? wert.ausnahmen : [])
    .map(datumOderNull).filter(Boolean))].sort().slice(0, 500);
  if (ausnahmen.length) sauber.ausnahmen = ausnahmen;
  return sauber;
};

/**
 * Ein einzelnes Vorkommen einer Serie auslassen oder wieder hereinnehmen.
 * Eigene Route, damit es aus der Tagesliste heraus mit einem Klick geht, ohne
 * das ganze Eintragsfenster zu bemuehen.
 */
const ausnahmeSetzen = ({ id, datum, wert }) => {
  const d = daten();
  const eintrag = d.entries.find((e) => e.id === id);
  if (!eintrag) return { fehler: 'Eintrag nicht gefunden.' };
  if (!eintrag.wiederholung) return { fehler: 'Nur eine Serie kann Ausnahmen haben.' };
  const tag = datumOderNull(datum);
  if (!tag) return { fehler: 'Für die Ausnahme wird der Tag gebraucht.' };

  const liste = new Set(eintrag.wiederholung.ausnahmen || []);
  if (wert) liste.add(tag); else liste.delete(tag);
  eintrag.wiederholung = wiederholungPruefen({ ...eintrag.wiederholung, ausnahmen: [...liste] });
  eintrag.geaendert = store.jetzt();
  entriesSchreiben(d.entries, d.kategorien);
  return { ok: true, ausnahmen: eintrag.wiederholung.ausnahmen || [] };
};
...
const daten = () => {
  ...
  return {
    ...
    vorlagen: Array.isArray(e.vorlagen) ? e.vorlagen : [],
...
const entriesSchreiben = (entries, kategorien, arten, vorlagen) => {
  // Arten und Vorlagen liegen in derselben Datei; wer sie nicht mitgibt, laesst sie wie sie sind.
  const alt = (!arten || !vorlagen) ? daten() : null;
  store.write('entries.json', {
    kategorien, arten: arten || alt.arten, vorlagen: vorlagen || alt.vorlagen, entries
  });
};
...
const alsVorkommen = (eintrag, vorkommenDaten, tag) => ({
  ...
  betrag: Number.isFinite(eintrag.betrag) ? eintrag.betrag : null,
...
/**
 * Was in einem Monat an Betraegen zusammenkommt. Ein Vorkommen zaehlt in dem
 * Monat, in dem es anfaengt — ein mehrtaegiger Eintrag also einmal, nicht je
 * Tag, und eine monatliche Serie in jedem Monat wieder.
 */
const monatsSumme = (entries, jahr, monat) => {
  const von = `${jahr}-${String(monat).padStart(2, '0')}-01`;
  const bis = D.plusTage(D.plusMonate(von, 1), -1);
  let plus = 0;
  let minus = 0;
  let anzahl = 0;
  for (const eintrag of entries) {
    if (!Number.isFinite(eintrag.betrag)) continue;
    for (const v of wdh.vorkommen(eintrag, von, bis)) {
      if (v.start < von || v.start > bis) continue;
      anzahl += 1;
      if (eintrag.betrag < 0) minus += eintrag.betrag; else plus += eintrag.betrag;
    }
  }
  const runden = (n) => Math.round(n * 100) / 100;
  return { plus: runden(plus), minus: runden(minus), saldo: runden(plus + minus), anzahl };
};
...
const eintragSpeichern = (koerper) => {
  ...
  const felder = {
    ...
    uhrzeit,
    text: inhalt,
    // Wo es stattfindet — frei formuliert, die Karte im Browser macht daraus den Weg.
    adresse: text(koerper.adresse, 300) || null,
    betrag: betragOderNull(koerper.betrag),
...
/* ---- Vorlagen -------------------------------------------------------------
   ...
   Was oefter, aber unregelmaessig anfaellt — Zahnarzt, Chorprobe, die Miete
   an wechselnden Tagen — muss nicht jedes Mal neu ausgefuellt werden. Eine
   Vorlage haengt am Namen und traegt die uebrigen Felder; im Eintragsfenster
   fuellt der Name sie nach. Gibt es zu einem Namen keine Vorlage, springt der
   juengste Eintrag gleichen Namens ein: was man schon einmal eingetragen hat,
   ist meist die beste Vorgabe. Datum und Text gehoeren nie zur Vorlage.     */

const vorlagenName = (wert) => text(wert, 300).toLowerCase().replace(/\s+/g, ' ');

/** Die Felder einer Vorlage aus einem Eintrag oder einem Formular. */
const vorlageFelder = (d, quelle) => {
  const datum = datumOderNull(quelle.datum);
  const bis = datumOderNull(quelle.datumBis);
  const felder = {
    uhrzeit: uhrzeitOderNull(quelle.uhrzeit),
    adresse: text(quelle.adresse, 300) || null,
    betrag: betragOderNull(quelle.betrag),
    kategorie: d.kategorien.some((k) => k.id === quelle.kategorie) ? quelle.kategorie : null,
    art: d.arten.some((a) => a.id === quelle.art) ? quelle.art : null,
    prioritaet: PRIOS.includes(quelle.prioritaet) ? quelle.prioritaet : 'mittel',
    // Mehrtaegiges merkt sich nur die Dauer; der Tag selbst kommt vom neuen Eintrag.
    tage: datum && bis && bis > datum ? D.differenzTage(datum, bis) : 0
  };
  if (Number.isFinite(quelle.tage) && quelle.tage > 0) felder.tage = Math.min(366, Math.round(quelle.tage));
  return felder;
};

const vorlageSpeichern = (koerper) => {
  const d = daten();
  const name = text(koerper.name || koerper.text, 300);
  if (!name) return { fehler: 'Für die Vorlage wird der Name gebraucht.' };
  const felder = vorlageFelder(d, koerper);

  // Ein Name, eine Vorlage: dieselbe Bezeichnung ueberschreibt die alte.
  const vorhanden = d.vorlagen.find((v) => vorlagenName(v.name) === vorlagenName(name));
  if (vorhanden) {
    Object.assign(vorhanden, felder, { name, geaendert: store.jetzt() });
    entriesSchreiben(d.entries, d.kategorien, d.arten, d.vorlagen);
    return { ok: true, vorlage: vorhanden, ersetzt: true };
  }
  const neu = { id: store.id('vl'), name, ...felder, erstellt: store.jetzt(), geaendert: store.jetzt() };
  entriesSchreiben(d.entries, d.kategorien, d.arten, [...d.vorlagen, neu].slice(-500));
  return { ok: true, vorlage: neu, ersetzt: false };
};

const vorlageLoeschen = (id) => {
  const d = daten();
  if (!d.vorlagen.some((v) => v.id === id)) return { fehler: 'Vorlage nicht gefunden.' };
  entriesSchreiben(d.entries, d.kategorien, d.arten, d.vorlagen.filter((v) => v.id !== id));
  return { ok: true };
};

/** Alle Namen, die das Eintragsfenster vorschlagen darf: Vorlagen zuerst, dann was schon eingetragen war. */
const vorlagenNamen = (d) => {
  const gesehen = new Set();
  const namen = [];
  const dazu = (name) => {
    const schluessel = vorlagenName(name);
    if (!schluessel || gesehen.has(schluessel)) return;
    gesehen.add(schluessel);
    namen.push(text(name, 300));
  };
  d.vorlagen.forEach((v) => dazu(v.name));
  [...d.entries]
    .sort((a, b) => String(b.geaendert || b.erstellt || '').localeCompare(String(a.geaendert || a.erstellt || '')))
    .forEach((e) => dazu(e.text));
  return namen.slice(0, 400);
};

/** Die Vorgabe zu einem Namen — aus der Vorlage, sonst aus dem juengsten Eintrag gleichen Namens. */
const vorlageZu = (d, name, datum) => {
  const schluessel = vorlagenName(name);
  if (!schluessel) return { vorlage: null, quelle: null };
  const vorlage = d.vorlagen.find((v) => vorlagenName(v.name) === schluessel);
  let felder = vorlage ? vorlageFelder(d, vorlage) : null;
  let quelle = vorlage ? 'vorlage' : null;
  if (!felder) {
    const eintrag = [...d.entries]
      .filter((e) => vorlagenName(e.text) === schluessel)
      .sort((a, b) => String(b.geaendert || b.erstellt || '').localeCompare(String(a.geaendert || a.erstellt || '')))[0];
    if (eintrag) { felder = vorlageFelder(d, eintrag); quelle = 'eintrag'; }
  }
  if (!felder) return { vorlage: null, quelle: null };
  const start = datumOderNull(datum);
  return {
    quelle,
    vorlage: { ...felder, datumBis: start && felder.tage ? D.plusTage(start, felder.tage) : null }
  };
};
...
const ruf = async (methode, pfad, koerper = {}) => {
  ...
  if (p === '/api/monat' && M === 'GET') {
    ...
    return { jahr, monat, gitter, tage, aufgaben, summe: monatsSumme(d.entries, jahr, monat), heute: store.heute() };
  }
  ...
  if (p === '/api/eintrag/ausnahme' && M === 'POST') return ausnahmeSetzen(koerper);
  if (p === '/api/vorlagen' && M === 'GET') {
    const d = daten();
    return { vorlagen: d.vorlagen, namen: vorlagenNamen(d) };
  }
  if (p === '/api/vorlage' && M === 'GET') return vorlageZu(daten(), url.searchParams.get('name'), url.searchParams.get('datum'));
  if (p === '/api/vorlage' && M === 'POST') return vorlageSpeichern(koerper);
  if (p === '/api/vorlage' && M === 'DELETE') return vorlageLoeschen(url.searchParams.get('id'));
  ...
  if (p === '/api/reime/kategorie' && M === 'DELETE') {
    return reimedaten.kategorieLoeschen(frage.get('id'));
  }

  if (p === '/api/reime/schub' && M === 'POST') return reimedaten.schubSetzen(koerper);
  if (p === '/api/reime/ordnen' && M === 'POST') return reimedaten.ordnen(koerper);
  if (p === '/api/reime/kopieren' && M === 'POST') return reimedaten.eintragKopieren(koerper);
  if (p === '/api/reime/kategorie/kopieren' && M === 'POST') return reimedaten.kategorieKopieren(koerper);
