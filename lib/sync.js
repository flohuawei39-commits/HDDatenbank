 ...
 *
 * Massgeblich ist der Stand bei GitHub — der lokale Spiegel ist ein Zwischenlager,
 * kein zweiter Bestand. Eine Ausnahme gibt es: liegt hier etwas, das drueben nie
 * angekommen ist (ohne Netz eingetragen), geht das zuerst hoch, statt beim Start
 * zu verschwinden. Sind beide Seiten unabhaengig weitergelaufen, entscheidet nicht
 * diese Datei, sondern der Mensch: es wird gemeldet und nichts ueberschrieben.
 */
const starten = async ({ passwort, ablage }) => {
  ...
  const fernDa = Boolean(fern && !fern.leer);
  // Wurde drueben das Passwort gewechselt, passt der Schluessel nicht mehr zum
  // Spiegel — dann ist hier nichts zu retten und der Fernstand gilt ohnehin.
  const salzGleich = !gespiegelt.salz || !fernDa || gespiegelt.salz === salz;
  const eigenes = Boolean(gespiegelt.schmutzig && Object.keys(gespiegelt.dateien || {}).length && salzGleich);
  const abgewichen = eigenes && fernDa && gespiegelt.kopfSha !== fern.kopfSha;

  const quelle = (fernDa && !eigenes) ? fern.dateien : gespiegelt.dateien;
  // Beim Konflikt bewusst den alten Kopf behalten: nur so lehnt GitHub ein
  // Hochschieben ab, statt den fremden Stand stillschweigend zu ueberschreiben.
  zustand.kopfSha = (fernDa && !abgewichen) ? fern.kopfSha : gespiegelt.kopfSha;
  ...
  zustand.schmutzig = eigenes;
  await spiegelSchreiben(quelle);

  // Was nur hier liegt, geht sofort hoch — danach gilt wieder der Stand von GitHub.
  let nachgetragen = null;
  if (eigenes && fernDa && !abgewichen) {
    nachgetragen = await sichern('Nach dem Start nachgetragen');
  }
  ...
  return {
    ...
    konflikt: abgewichen || Boolean(nachgetragen && nachgetragen.konflikt),
    ungesichert: zustand.schmutzig,
    standZeit: gespiegelt.zeit || null,
...
const spiegelSchreiben = (dateien) => spiegel.standSetzen({
  ...
  salz: zustand.salz,
  // Nur fuer die Anzeige „Stand vom ...", wenn ohne Verbindung gestartet wird.
