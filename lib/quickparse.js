...
const parse = (eingabe, kategorien = [], heute = null) => {
  ...
  /* !frist / !termin / !aufgabe — die Art des Eintrags. `!frist` gab es schon,
     bevor es Arten gab, und setzt deshalb zusaetzlich weiterhin `istFrist`.
     Vor der Prioritaet auswerten: sonst schluckt deren allgemeineres `!wort`
     diese Marken weg. */
  const frist = raus(rest, /\s!frist\b/i);
  if (frist.treffer) { istFrist = true; art = 'art_frist'; rest = frist.rest; }

  const termin = raus(rest, /\s!termin\b/i);
  if (termin.treffer) { art = 'art_termin'; rest = termin.rest; }

  const aufgabe = raus(rest, /\s!aufgabe\b/i);
  if (aufgabe.treffer) { art = 'art_aufgabe'; rest = aufgabe.rest; }
