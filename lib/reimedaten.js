...
const gruppeAufbereiten = (gruppe, d) => {
  ...
  const eintraege = (Array.isArray(gruppe.eintraege) ? gruppe.eintraege : []).map((e) => {
    ...
    return {
      ...
      text: e.text,
      silben: kette,
      versatz: (treffer.laenge ? treffer.a - treffer.b : 0) + schub,
      schub,
      treffer
    };
  });
  ...
  const zeilen = [
    { id: gruppe.id, kopf: true, text: gruppe.kopf, silben: kopfKette, versatz: kopfSchub, schub: kopfSchub },
    ...eintraege
  ];
...
const ausrichten = (zeilen) => {
  ...
  zeilen.forEach((z, i) => {
    ...
    const gerechnet = i === 0 ? 0 : (() => {
      ...
      return t.laenge ? letzter + (t.a - t.b) : letzter;
    })();
    letzter = gerechnet;
...
export {
  ...
  wortSpeichern, kategorieSpeichern, kategorieLoeschen, anzeigeSpeichern, schubSetzen,
